(function () {
  ...
  var MATCH_RETRY_COUNT = 2;
  var MATCH_RETRY_DELAYS = [500, 1500];

  var KEY = {
    token: 'kp_token',
    refresh: 'kp_refresh',
    lastStatus: 'kp_sync0110_last_status',
    report: 'kp_sync0110_bookmarks_report',
    tokenStatus: 'kp_sync0110_token_status',
    cleanupReport: 'kp_sync0110_lampa_cleanup_report',
    matchReport: 'kp_sync0110_match_report',
    matchLimit: 'kp_sync0110_match_limit',
    tmdbProxyEmail: 'kp_sync0110_tmdb_proxy_email',
    tmdbDiag: 'kp_sync0110_tmdb_diag',
    preferredTmdbProvider: 'kp_sync0110_preferred_tmdb_provider'
  };

  if (window.KinoPubSync0110 && window.KinoPubSync0110.version === VERSION) return;
  ...
  function lang(key) {
    var ru = {
      component: 'KinoPUB Sync 0.1.10',
      sep: '— Проверка и чтение KinoPUB —',
      sep_descr: 'Alpha-сборка: универсальная диагностика чтения KinoPUB и read-only сопоставление с TMDB/Lampa. v0.1.10 использует page+perpage=48 как основной размер страницы, потому что веб-портал KinoPUB показывает 48 карточек на HTML-страницу. Ничего не импортирует в Lampa и ничего не меняет в KinoPUB.',
      ...
      read_bookmarks_descr: 'Читает все доступные папки и элементы закладок авторизованного аккаунта KinoPUB. v0.1.10 сначала пробует page+perpage=48, затем универсальные fallback-стратегии. Ничего не записывает в Lampa.',
      ...
      run_match_descr: 'Запускает read-only проверку кандидатов к будущему импорту. Сначала делает быстрый TMDB preflight с приоритетом сохранённого рабочего proxy. Импорт-кандидат появляется только если получена настоящая TMDB/Lampa-карточка по IMDb и тип movie/tv совпал. Kinopoisk-only карточки блокируются до отдельного резолвера.',
      retry_api_errors: 'Повторить только ошибки API',
      retry_api_errors_descr: 'Повторяет сопоставление только для элементов из последнего отчёта со статусом blocked_api_error. Полезно после временного сбоя proxy/TMDB, чтобы не гонять весь список заново.',
      ...
      no_api_error_items: 'В последнем отчёте нет элементов blocked_api_error для повтора.',
  ...
  function storageGetAny(keys, def) {
    for (var i = 0; i < keys.length; i++) {
      var v = storageGet(keys[i], null);
  ...
  function buildReportSummary(report) {
    ...
    lines.push('Важно: v0.1.10 ничего не импортирует в Lampa и ничего не меняет в KinoPUB. Очистка данных Lampa находится в основных настройках плагина и выполняется только после сканирования и подтверждения.');
    return lines.join('\n');
  }
  ...
  function readBookmarks() {
    ...
    var report = {
      ...
      api: { host: API_HOST, endpoints: ['/v1/bookmarks', '/v1/bookmarks/<folder_id>', '/v1/bookmarks/view?folder=<folder_id>'], retrievalAudit: 'Generic multi-strategy pagination probe. v0.1.10 starts with page/perpage=48 because KinoPUB web portal pages contain 48 cards; this is site-behavior-based, not account-specific. Fallbacks include page/perpage=50/100/25, page/limit, page/per_page, page/page_size, offset/limit, skip/limit, optional sort/order variants. No account-specific counts, folder names or item ids are hardcoded.' },
      folders: [],
      catalog: [],
      rawSamples: [],
  ...
  function getReport() { return storageGetAny([KEY.report, 'kp_sync019_bookmarks_report', 'kp_sync018_bookmarks_report', 'kp_sync017_bookmarks_report', 'kp_sync016_bookmarks_report'], null); }
  function reportText() { var r = getReport(); return r ? JSON.stringify(r, null, 2) : lang('report_missing'); }
  function showReport() { var r = getReport(); if (!r) { noty(lang('report_missing')); return; } showText('KinoPUB Sync 0.1.10', r.summaryText || reportText()); }
  function copyReport() { var text = reportText(); return copyText(text).then(function () { noty(lang('copied')); }).catch(function () { noty(lang('copy_failed')); showText('KinoPUB Sync 0.1.10 — отчёт', text); }); }
  ...
  function orderProxyUrls(urls, preferred) {
    preferred = String(preferred || '');
    if (!preferred || preferred.indexOf('tmdb_proxy') !== 0) return urls || [];
    var first = [], rest = [];
    for (var i = 0; i < (urls || []).length; i++) {
      if (String(urls[i].provider || '') === preferred) first.push(urls[i]);
      else rest.push(urls[i]);
    }
    return first.concat(rest);
  }

  function tmdbApiProxy(path, params, preferred) {
    var urls = orderProxyUrls(tmdbProxyUrls(path, params), preferred);
    var errors = [];
    function next(i) {
  ...
  function rememberPreferredTmdbProvider(provider) {
    provider = String(provider || '');
    if (provider && provider.indexOf('tmdb_proxy') === 0) storageSet(KEY.preferredTmdbProvider, provider);
  }

  function preferredTmdbProvider() {
    var stored = storageGet(KEY.preferredTmdbProvider, '');
  ...
  function tmdbApi(path, params) {
    ...
    if (preferred.indexOf('tmdb_proxy') === 0) {
      return tmdbApiProxy(path, params, preferred).catch(function (proxyErr) {
        return tmdbApiLampa(path, params).catch(function (lampaErr) {
          return Promise.reject({ status: 'all_tmdb_methods_failed', provider: 'auto', preferred: preferred, proxy: errorSummary(proxyErr), lampa: errorSummary(lampaErr) });
        });
      });
    }
    return tmdbApiProxy(path, params, 'tmdb_proxy_cub_rip').catch(function (proxyErr) {
      return tmdbApiLampa(path, params).catch(function (lampaErr) {
  ...
  function tmdbDiagnosticProbe(imdb) {
    ...
    function finishOk(provider, json) {
      ...
      rememberPreferredTmdbProvider(diag.provider);
    ...
    var preferred = preferredTmdbProvider();
    var proxies = orderProxyUrls(tmdbProxyUrls(path, params), preferred || 'tmdb_proxy_cub_rip');
    var methods = [];
    for (var pi = 0; pi < proxies.length; pi++) methods.push({ kind: 'proxy', provider: proxies[pi].provider, url: proxies[pi].url });
    if (diag.tmdbApiPresent) methods.push({ kind: 'lampa', provider: 'lampa_tmdb_api', url: '' });
    else diag.methods.push({ provider: 'lampa_tmdb_api', url: '', ok: false, durationMs: 0, skipped: true, error: { http: 0, status: 'no_lampa_tmdb_api', message: 'Lampa.TMDB.api is not available' }, rawCounts: null });

    var chain = Promise.reject({ status: 'start' });
    function tryMethod(m) {
      return chain.catch(function () {
        return record(m.provider, m.url, function () {
          if (m.kind === 'lampa') return tmdbApiLampa(path, params);
          return fetchJsonUrl(m.url, m.provider);
        }).then(function (json) { return finishOk(m.provider, json); });
      });
    }
    for (var mi = 0; mi < methods.length; mi++) chain = tryMethod(methods[mi]);
    return chain.catch(finishBad);
  }
  ...
  function getTmdbDiag() { return storageGetAny([KEY.tmdbDiag, 'kp_sync019_tmdb_diag', 'kp_sync018_tmdb_diag', 'kp_sync017_tmdb_diag', 'kp_sync016_tmdb_diag'], null); }
  ...
  function buildTmdbDiagText(diag) {
    ...
    if (diag.preferredProvider) lines.push('Сохранённый preferred provider: ' + diag.preferredProvider);
  ...
  function matchOneItem(item) {
    ...
    function attempt(n) {
      ...
      return tmdbFindByImdb(imdb).then(function (json) {
        var res = analyzeTmdbFind(json, item, imdb);
        attempts.push({ attempt: n + 1, ok: true, durationMs: Date.now() - astart, provider: res.provider || res.match_source || '' });
        res.retry_count = n;
        res.attempts = attempts;
        res.duration_ms = Date.now() - started;
        return res;
      }).catch(function (err) {
        attempts.push({ attempt: n + 1, ok: false, durationMs: Date.now() - astart, error: errorSummary(err) });
        if (n < MATCH_RETRY_COUNT) {
          return sleep(MATCH_RETRY_DELAYS[n] || 500).then(function () { return attempt(n + 1); });
        }
        var failed = baseMatchResult(item, imdb);
        failed.error = errorSummary(err);
        failed.retry_count = n;
  ...
  function buildMatchSummary(report) {
    ...
    if (report.tmdbPreflight) lines.push('TMDB preflight: ' + (report.tmdbPreflight.ok ? 'OK' : 'ERROR') + ', ' + (report.tmdbPreflight.durationMs || 0) + ' ms' + (report.tmdbPreflight.provider ? ', provider: ' + report.tmdbPreflight.provider : ''));
    if (report.performance) {
      lines.push('Скорость: всего ' + (report.performance.durationMs || 0) + ' ms, среднее ' + (report.performance.avgMsPerChecked || 0) + ' ms/элемент');
      lines.push('Провайдеры: ' + JSON.stringify(report.performance.providers || {}));
      lines.push('Retry: элементов с повтором ' + (report.performance.retriedItems || 0) + ', всего retry ' + (report.performance.retryCount || 0));
    }
  ...
  function createMatchStats(total) {
    return {
      total: total || 0,
      checked: 0,
      matched: 0,
  ...
  function runSelectedMatchAudit(selected, opts) {
    opts = opts || {};
    selected = selected || [];
    ...
    var report = {
      ...
      edition: EDITION,
      generatedAt: nowIso(),
      source: opts.source || 'KinoPUB -> TMDB/Lampa read-only import readiness audit',
      basedOnBookmarksReportAt: opts.basedOnBookmarksReportAt || '',
      previousMatchReportAt: opts.previousMatchReportAt || '',
      tokensIncluded: false,
      mode: {
        name: opts.modeName || 'match_audit',
        requestedTotal: opts.requestedTotal || selected.length,
        checkedTotal: selected.length,
        limit: opts.limit || 0,
        timeoutMs: MATCH_TIMEOUT_MS,
        concurrency: MATCH_CONCURRENCY,
        abortApiErrors: MATCH_ABORT_API_ERRORS,
        retryCount: MATCH_RETRY_COUNT,
        retryDelaysMs: MATCH_RETRY_DELAYS.slice(0),
        preferredProvider: preferredTmdbProvider() || '',
        note: opts.note || 'Only resolved TMDB/Lampa cards found by IMDb are import candidates. Kinopoisk ID is diagnostic unless a reliable Kinopoisk->TMDB/Lampa resolver is implemented. No import. v0.1.10 uses fast TMDB provider preference, retries transient API errors, and can repeat only blocked_api_error items.'
      },
      tmdbPreflight: null,
      aborted: false,
      abortReason: '',
      ...
      performance: {
        ...
        durationMs: 0,
        avgMsPerChecked: 0,
        preferredProviderBefore: preferredTmdbProvider() || '',
        preferredProviderAfter: '',
        providers: {},
        retryCount: 0,
        retriedItems: 0
      },
      stats: createMatchStats(selected.length),
      results: [],
      importCandidates: [],
      blockedItems: [],
      problemItems: []
    ...
    function finishMatchReport(message) {
      ...
      report.performance.finishedAt = nowIso();
      report.performance.durationMs = Date.now() - startedAt;
      report.performance.avgMsPerChecked = report.stats.checked ? Math.round(report.performance.durationMs / report.stats.checked) : 0;
      report.performance.preferredProviderAfter = preferredTmdbProvider() || '';
      report.summaryText = buildMatchSummary(report);
      storageSet(KEY.matchReport, report);
      setStatus(message || (lang('match_done') + ': import candidates ' + report.stats.importCandidates + '/' + report.stats.checked + ', blocked ' + report.stats.blocked + ', avg ' + report.performance.avgMsPerChecked + ' ms'));
    ...
    return tmdbDiagnosticProbe('tt0111161').then(function (diag) {
      ...
      return runPool(selected, MATCH_CONCURRENCY, function (item, idx) {
        ...
        return matchOneItem(item).then(function (res) {
          ...
          var provider = String((res && (res.provider || res.match_source)) || 'unknown');
          report.performance.providers[provider] = (report.performance.providers[provider] || 0) + 1;
          var rc = parseInt(res && res.retry_count || 0, 10) || 0;
          report.performance.retryCount += rc;
          if (rc > 0) report.performance.retriedItems++;
          progress++;
          ...
          if (apiErrorStreak >= MATCH_ABORT_API_ERRORS) {
            ...
            report.abortRecommendation = 'Получено ' + apiErrorStreak + ' подряд ошибок TMDB/Lampa API даже после retry. Проверка остановлена, чтобы не ждать десятки минут. Проверьте VPN/доступ к TMDB или выполните диагностику TMDB.';
          }
  ...
  function runMatchAudit() {
    ...
    return runSelectedMatchAudit(selected, {
      ...
      limit: limit,
      modeName: limit > 0 ? 'limited_match_audit' : 'full_match_audit'
    });
  }

  function apiErrorItemsFromReport(report) {
    var out = [];
    var seen = {};
    var src = report && (report.problemItems || report.blockedItems || report.results) || [];
    for (var i = 0; i < src.length; i++) {
      var r = src[i] || {};
      if (String(r.status || '') !== 'blocked_api_error') continue;
      var item = r.kinopub || {};
      var id = String(item.id || r.imdb_query || i);
      if (seen[id]) continue;
      seen[id] = true;
      out.push(item);
    }
  ...
  function runRetryApiErrors() {
    var prev = getMatchReport();
    var selected = apiErrorItemsFromReport(prev);
    if (!selected.length) { noty(lang('no_api_error_items')); return Promise.resolve(false); }
    return runSelectedMatchAudit(selected, {
      source: 'KinoPUB -> TMDB/Lampa retry only blocked_api_error',
      basedOnBookmarksReportAt: prev && prev.basedOnBookmarksReportAt || '',
      previousMatchReportAt: prev && prev.generatedAt || '',
      requestedTotal: selected.length,
      limit: selected.length,
      modeName: 'retry_blocked_api_error',
      note: 'Read-only retry of items that had blocked_api_error in the previous match report. No import. Successful items become importCandidates; failed items remain blockedItems.'
    });
  }

  function getMatchReport() { return storageGetAny([KEY.matchReport, 'kp_sync019_match_report', 'kp_sync018_match_report', 'kp_sync017_match_report'], null); }
  function matchReportText() { var r = getMatchReport(); return r ? JSON.stringify(r, null, 2) : lang('match_missing'); }
  function showMatchReport() { var r = getMatchReport(); if (!r) { noty(lang('match_missing')); return; } showText('KinoPUB Sync 0.1.10 — готовность импорта', r.summaryText || matchReportText()); }
  function copyMatchReport() { var text = matchReportText(); return copyText(text).then(function () { noty(lang('copied')); }).catch(function () { noty(lang('copy_failed')); showText('KinoPUB Sync 0.1.10 — отчёт готовности импорта', text); }); }
  function clearMatchReport() { storageRemove(KEY.matchReport); storageRemove('kp_sync019_match_report'); storageRemove('kp_sync018_match_report'); storageRemove('kp_sync017_match_report'); noty(lang('match_report_cleared')); }
  ...
  function showCleanupReport() { var r = cleanupReport(); if (!r) { noty(lang('cleanup_missing')); return; } showText('KinoPUB Sync 0.1.10 — очистка Lampa', JSON.stringify(r, null, 2)); }
  function copyCleanupReport() { var text = cleanupReportText(); return copyText(text).then(function () { noty(lang('copied')); }).catch(function () { noty(lang('copy_failed')); showText('KinoPUB Sync 0.1.10 — очистка Lampa', text); }); }
  ...
  function addSettings() {
    try {
      ...
      addParam('kp_sync0110_sep_main', 'title', '', '', lang('sep'), lang('sep_descr'));
      addParam(KEY.lastStatus, 'input', storageGet(KEY.lastStatus, '') || '', '', lang('status'), lang('status_descr'));
      addParam('kp_sync0110_action_check_token', 'button', '', '', lang('check_token'), lang('check_token_descr'), function () { checkToken(); });
      addParam('kp_sync0110_action_read_bookmarks', 'button', '', '', lang('read_bookmarks'), lang('read_bookmarks_descr'), function () { readBookmarks(); });
      addParam('kp_sync0110_action_show_report', 'button', '', '', lang('show_report'), lang('show_report_descr'), function () { showReport(); });
      addParam('kp_sync0110_action_copy_report', 'button', '', '', lang('copy_report'), lang('copy_report_descr'), function () { copyReport(); });
      addParam('kp_sync0110_action_clear_report', 'button', '', '', lang('clear_report'), lang('clear_report_descr'), function () { clearReport(); });
      addParam('kp_sync0110_sep_match', 'title', '', '', lang('sep_match'), lang('sep_match_descr'));
      ...
      addParam('kp_sync0110_action_test_tmdb', 'button', '', '', lang('test_tmdb'), lang('test_tmdb_descr'), function () { diagnoseTmdbApi(); });
      addParam('kp_sync0110_action_show_tmdb_diag', 'button', '', '', lang('show_tmdb_diag'), lang('show_tmdb_diag_descr'), function () { showTmdbDiag(); });
      addParam('kp_sync0110_action_copy_tmdb_diag', 'button', '', '', lang('copy_tmdb_diag'), lang('copy_tmdb_diag_descr'), function () { copyTmdbDiag(); });
      addParam('kp_sync0110_action_run_match', 'button', '', '', lang('run_match'), lang('run_match_descr'), function () { runMatchAudit(); });
      addParam('kp_sync0110_action_retry_api_errors', 'button', '', '', lang('retry_api_errors'), lang('retry_api_errors_descr'), function () { runRetryApiErrors(); });
      addParam('kp_sync0110_action_show_match', 'button', '', '', lang('show_match'), lang('show_match_descr'), function () { showMatchReport(); });
      addParam('kp_sync0110_action_copy_match', 'button', '', '', lang('copy_match'), lang('copy_match_descr'), function () { copyMatchReport(); });
      addParam('kp_sync0110_action_clear_match', 'button', '', '', lang('clear_match'), lang('clear_match_descr'), function () { clearMatchReport(); });
      addParam('kp_sync0110_sep_cleanup', 'title', '', '', lang('sep_cleanup'), lang('sep_cleanup_descr'));
      addParam('kp_sync0110_action_scan_cleanup', 'button', '', '', lang('scan_bad_lampa'), lang('scan_bad_lampa_descr'), function () { scanOldLampaBookmarks(); });
      addParam('kp_sync0110_action_show_cleanup', 'button', '', '', lang('show_cleanup'), lang('show_cleanup_descr'), function () { showCleanupReport(); });
      addParam('kp_sync0110_action_copy_cleanup', 'button', '', '', lang('copy_cleanup'), lang('copy_cleanup_descr'), function () { copyCleanupReport(); });
      addParam('kp_sync0110_action_apply_cleanup', 'button', '', '', lang('clear_bad_lampa'), lang('clear_bad_lampa_descr'), function () { applyOldLampaCleanup(); });
    } catch (e) { log('settings failed', e && e.message); }
  }

  function start() {
    if (window.KinoPubSync0110 && window.KinoPubSync0110._started) return;
    window.KinoPubSync0110 = {
      _started: true,
      version: VERSION,
      edition: EDITION,
      ...
      runRetryApiErrors: runRetryApiErrors,
